...
def phase_trade(ctx: Ctx, keys: List[str], token: Optional[str],
                ...
                tps: int = 10, chunk_size: int = 50):
    info(f"=== ZERO-WAIT FIREHOSE TRADE ({tps} TX/SEC | ${lo}-${hi} per wallet) ===")
    ...
    job_queue = queue.Queue(maxsize=200)
    receipt_queue = queue.Queue()
    stop_producer = threading.Event()
    stop_receipts = threading.Event()
    results = {"sent": 0, "skip-funds": 0, "skip-allowance": 0, "error": 0}

    # -----------------------------------------------------------------------
    # 1. PRODUCER THREAD: Prefetches chunks ahead of time so queue never starves
    # -----------------------------------------------------------------------
    def producer_worker():
        total_chunks = (len(keys) + chunk_size - 1) // chunk_size
        for chunk_idx, i in enumerate(range(0, len(keys), chunk_size)):
            # Refresh token pool every 45s
            if time.time() - last_refresh[0] > 45 or not active_pool:
                refresh_token_pool()

            with pool_lock:
                current_pool = list(active_pool)
            ...
            # JIT Preflight for this 50-wallet chunk
            try:
            ...
            for pk in chunk_keys:
                ...
                # 80% newest 10 tokens, 20% broader active pool
                if len(current_pool) > 5 and random.random() < 0.80:
                    tgt = random.choice(current_pool[:min(10, len(current_pool))])
                else:
                    tgt = random.choice(current_pool)
                ...
                job = (acct, ROUTER, data, 320_000, f"buy {amount} USDC",
                       {"token": tgt, "amount_usdc": amount})
                job_queue.put(job)
        stop_producer.set()

    # -----------------------------------------------------------------------
    # 2. BACKGROUND RECEIPT DRAINER: Collects receipts without blocking firehose
    # -----------------------------------------------------------------------
    def receipt_worker():
        pending_hashes = []
        while not stop_receipts.is_set() or not receipt_queue.empty() or pending_hashes:
            while not receipt_queue.empty():
                try:
                    pending_hashes.append(receipt_queue.get_nowait())
                except queue.Empty:
                    break
            if pending_hashes and ctx.send:
                batch = pending_hashes[:100]
                pending_hashes = pending_hashes[100:]
                try:
                    ctx.sender.batch_wait(batch, timeout=20)
                except Exception:
                    pass
            time.sleep(3.0)

    t_prod = threading.Thread(target=producer_worker, daemon=True)
    t_rcpt = threading.Thread(target=receipt_worker, daemon=True)
    t_prod.start()
    t_rcpt.start()

    # -----------------------------------------------------------------------
    # 3. FIREHOSE BROADCASTER: Fires exactly `tps` tx every 1.0 second on clock
    # -----------------------------------------------------------------------
    total_broadcast = 0
    while not stop_producer.is_set() or not job_queue.empty():
        batch = []
        while len(batch) < tps and not job_queue.empty():
            try:
                batch.append(job_queue.get_nowait())
            except queue.Empty:
                break
        ...
        t0 = time.perf_counter()
        hashes, tx_by_wallet = ctx.sender.batch_send(batch)
        total_broadcast += len(hashes)
        results["sent"] += len(hashes)

        for h in hashes:
            receipt_queue.put(h)

        for (acct, _, _, _, label, meta) in batch:
        ...
        log(f"{GREEN}[FIREHOSE]{RESET} Fired {len(hashes)} txs in {elapsed:.2f}s "
            f"| Total fired: {total_broadcast} | Queue: {job_queue.qsize()}")
        time.sleep(sleep_needed)

    t_prod.join(timeout=5)
    stop_receipts.set()
    t_rcpt.join(timeout=10)

    info(f"trade complete: {results} (total fired: {total_broadcast})")
...
def main():
    ...
    ap.add_argument("--tps", type=int, default=10, help="Transactions per second firehose rate (default 10)")
    ...
    try:
        def run_cycle(cycle_num: int = 1):
            ...
            if new_deployed:
                info(f"deployment confirmed; immediately starting buy wave on {len(new_deployed)} tokens...")
        ...
        if args.auto or args.loop:
            ...
            if args.collect_fees:
                phase_fees(ctx)
            if args.trade:
                phase_trade(ctx, keys, args.token, args.trade_min, args.trade_max, tps=args.tps)
            if args.exit:
